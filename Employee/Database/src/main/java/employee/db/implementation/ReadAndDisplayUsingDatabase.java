...
import employee.db.entity.EmployeeAddress;
import employee.db.entity.EmployeeBasicDetails;
...
public class ReadAndDisplayUsingDatabase implements InputEmployeeDetails {
    ...
    public Employee saveAll(Employee employee) {
        ...
        try {
            ...
            String insertTemporaryAddress = "insert into CurrentAddress(EMPLOYEE_ID,HOUSE_NAME,STREET_NAME,CITY_NAME,STATE_NAME,PIN_CODE) values (?,?,?,?,?,?)";
            ...
            String insertPermanentAddress = "insert into Address(EMPLOYEE_ID,HOUSE_NAME,STREET_NAME,CITY_NAME,STATE_NAME,PIN_CODE) values (?,?,?,?,?,?)";
    ...
    public Employee displayRequired(int employeeID) {
        ...
        try {
            connection=EstablishConnection.EstablishConnection();
            String findByID="SELECT * FROM employeebasicdetails ebd INNER JOIN currentaddress ta ON ebd.employee_id = ta.employee_id INNER JOIN address pa ON ebd.employee_id = pa.employee_id WHERE ebd.employee_id = ?";
...
//                employeeBasicDetails.setTemporaryEmployeeAddress(permEmployeeAddress);
    ...
    public List<Employee> displayBasedOnPinCode(int temporaryPincode) {
        try {
            ...
            EmployeeAddress tempEmployeeAddress=new EmployeeAddress();
            EmployeeAddress permEmployeeAddress=new EmployeeAddress();
            String findByID="SELECT * FROM employeebasicdetails ebd INNER JOIN currentaddress ta ON ebd.employee_id = ta.employee_id INNER JOIN address pa ON ebd.employee_id = pa.employee_id WHERE ta.pin_code = ?";
            preparedStatement=connection.prepareStatement(findByID);
...
//                employeeAddress.setTemporaryPinCode(resultSet.getInt("pin_code"));
    ...
    public List<Employee> displayAll() {
        ...
        try {
            connection=EstablishConnection.EstablishConnection();
            String findAll="SELECT * FROM employeebasicdetails ebd INNER JOIN currentaddress ta ON ebd.employee_id = ta.employee_id INNER JOIN address pa ON ebd.employee_id = pa.employee_id ";
